  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <header className="border-b border-slate-200 pb-6">
        <h2 className="text-headline-lg font-headline-lg text-slate-900">Visão Geral do Negócio</h2>
        <p className="text-body-md font-body-md text-slate-600">Métricas de faturamento, lucro e estoque em tempo real.</p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-label-sm font-label-sm text-slate-700 uppercase tracking-wider">Faturamento</p>
            <p className="text-display-lg font-display-lg text-slate-900">R$ {allTimeStats.revenue.toFixed(2)}</p>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-label-sm font-label-sm text-slate-700 uppercase tracking-wider">Lucro Estimado</p>
            <p className="text-display-lg font-display-lg text-emerald-600">R$ {allTimeStats.totalProfit.toFixed(2)}</p>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-label-sm font-label-sm text-slate-700 uppercase tracking-wider">Valor em Estoque</p>
            <p className="text-display-lg font-display-lg text-slate-900">R$ {totalStockValue.toFixed(2)}</p>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-4 relative">
          ...
          <div>
            <p className="text-label-sm font-label-sm text-slate-700 uppercase tracking-wider">Alertas de Reposição</p>
            <p className="text-display-lg font-display-lg text-slate-900">{products.filter(p => p.stock < 5).length}</p>
          </div>
        </div>
      </div>
            ...
            <div className="space-y-4">
              {topProducts.map((p, i) => (
                <div key={p.id} className="flex items-center gap-3 p-2 hover:bg-slate-50 rounded-xl transition-colors border border-transparent hover:border-slate-100">
                  <span className="text-debug-mono font-debug-mono text-slate-400 w-4">{i + 1}</span>
                  ...
                  <p className="text-debug-mono font-debug-mono font-bold text-emerald-600">R$ {p.price.toFixed(2)}</p>
                </div>
              ))}
            </div>
          ) : (
